...
public class BezierFigure extends AbstractAttributedFigure {
    ...
    public boolean contains(Point2D.Double p) {
        ...
        Boolean closedOrFilledContainsReturn = closedOrFilledContains(p);
        if (closedOrFilledContainsReturn != null) return closedOrFilledContainsReturn;
        Boolean unclosedContainsReturn = unclosedContains(p, tolerance);
        if (unclosedContainsReturn != null) return unclosedContainsReturn;
        return false;
    }

    private Boolean closedOrFilledContains(Point2D.Double p){
    ...
    private Boolean unclosedContains(Point2D.Double p, double tolerance) {
        if (!isClosed()) {
            if (getCappedPath().outlineContains(p, tolerance)) {
                return true;
            }
            if (containsIfStartDecorator(p, tolerance)) return true;
            return containsIfStopDecorator(p, tolerance);
        }
        return null;
    }
    ...
    public boolean handleMouseClick(Point2D.Double p, MouseEvent evt, DrawingView view) {
        if (evt.getClickCount() == 2 && view.getHandleDetailLevel() % 2 == 0) {
            ...
            if (index != -1) {
                ...
                fireUndoableEditHappened(new BezierFigureUndoableEdit(index, newNode));
                changed();
                evt.consume();
                return true;
            }
    ...
    private class BezierFigureUndoableEdit extends AbstractUndoableEdit {
        private final int index;
        private final BezierPath.Node newNode;

        public BezierFigureUndoableEdit(int index, BezierPath.Node newNode) {
            this.index = index;
